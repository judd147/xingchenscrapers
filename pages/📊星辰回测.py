    ...
    for i in range(len(tabs)):
        with tabs[i]:
            ...
            with st.form("search_history_{i}".format(i=i)):
                ...
                if submit4search:
                    if fuzzy:
                        df_team_history = df_season[df_season['比赛'].str.contains(team2search)]
                    else:
                        df_metric = clean_history(df_season)
                        df_temp_teams = find_recommend(df_metric)
                        df_team_history = df_temp_teams[df_temp_teams['team']==(team2search)].iloc[0:,:14]
...
def read_file(data):
    df = pd.read_excel(data, sheet_name = 1, converters = {'年': str, '盘口': str, '竞彩': str, '比分': str, '主赔': float, '客赔': float})
...
def judge(new_score, hand, home, away, deep, signal, home_odds, away_odds):
    ...
    if abs(home_hand_diff) == 0.25: # 赢半/输半
        ...
        coef = 0
    return_pct = lambda coef, odds: coef * odds if coef != 0 else (0.55) # lambda function to calculate return percentage
...
def search(df, opt1):
    ...
    if opt1:
        history = True
    dfb = pd.DataFrame(columns=['开球时间','联赛','比赛','让球方','盘口','模型','平均概率','最长遗漏','高频比分','频率','算法数量','正误','模拟盈亏'])
    ...
    for index, row in df[df['H'].isnull() & df['盘口'].notnull()].iterrows():
        ...
        #新比赛
            ...
            if sum(uppr_count)/algo >= 0.5 and algo > 1:
                ...
                if history:
                    TF, profit = judge(new_score, num_hand, home, away, deep, 'uppr', home_odds, away_odds)
                    if TF:
                        outcome = '\u2714'
                    else:
                        outcome = '\u2716'
            ...
            #下盘
                ...
                if history:
                    TF, profit = judge(new_score, num_hand, home, away, deep, 'down', home_odds, away_odds)
                    if TF:
                        outcome = '\u2714'
                    else:
                        outcome = '\u2716'
        ...
        if total < 2:
            ...
            mix_total = len(mixed)
            temp_home = mixed[mixed['盘口'].str.contains('\-')|mixed['注释'].str.contains('\-')]
            temp_away = mixed[mixed['盘口'].str.contains('\+')|mixed['注释'].str.contains('\+')]
            if deep:
            ...
            if mix_total >= 10:
                ...
                if total > 0:
                    ...
                    temp_home = m_result[m_result['盘口'].str.contains('\-')|m_result['注释'].str.contains('\-')]
                    temp_away = m_result[m_result['盘口'].str.contains('\+')|m_result['注释'].str.contains('\+')]
                    if deep:
                ...
                if total >= 10:
                    ...
                    if total > 0:
                        ...
                        temp_home = m_result3[m_result3['盘口'].str.contains('\-')|m_result3['注释'].str.contains('\-')]
                        temp_away = m_result3[m_result3['盘口'].str.contains('\+')|m_result3['注释'].str.contains('\+')]
                        if deep:
                            p_uppr = ((len(temp_home[(temp_home['H']-temp_home['A']) > 1])+len(temp_away[(temp_away['A']-temp_away['H']) > 1])+1)/(total+2))*100
                            p_down = 100-p_uppr
                        else:
        ...
        if roll:
            roll = False
            if home:
                result1 = result0[result0['盘口'].str.contains('\-')|result0['注释'].str.contains('\-')]
            elif away:
                result1 = result0[result0['盘口'].str.contains('\+')|result0['注释'].str.contains('\+')]
            total = len(result1)
            if total > 0:
